...
function wrapMethodHanderForErrors(name, originalHandler, methodMap) {
    methodMap[name] = function() {
        Meteor.logger.profile('UserID: ' + (this.userId ? this.userId : 'Unknown') + ' Method: ' + name + ", arguments: " + JSON.stringify(arguments));

        try{
            var method = originalHandler.apply(this, arguments);
